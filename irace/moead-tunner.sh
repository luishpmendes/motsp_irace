#!/bin/bash
###############################################################################
# iRace Target Runner Script for MOEA/D Solver (MOTSP)
#
...
mkdir -p "${PROJECT_DIR}/irace/irace_moead"
TMP_DIR="${PROJECT_DIR}/irace/irace_moead"
...
# Run the MOEA/D solver with the given configuration
if ! "$SOLVER" \
    --instance "$INSTANCE" \
    --seed "$SEED" \
