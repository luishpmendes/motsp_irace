#!/bin/bash
###############################################################################
# iRace Target Runner Script for NS-BRKGA Solver (MOTSP)
#
...
#   $5+: Parameter settings (e.g., --population-size-factor 50 --mutation-probability 0.1 ...)
#
...
mkdir -p "${PROJECT_DIR}/irace/irace_nsbrkga"
TMP_DIR="${PROJECT_DIR}/irace/irace_nsbrkga"
...
echo "Executing command: $SOLVER --instance $INSTANCE --seed $SEED --time-limit $TIME_LIMIT --pareto $PARETO_FILE ${PARAMS[@]}"

# Run the NS-BRKGA solver with the given configuration
if ! "$SOLVER" \
    --instance "$INSTANCE" \
    --seed "$SEED" \
...
if ! "$HV_CALC" \
    ...
    > /dev/null 2>&1; then
    # Hypervolume computation failed
    echo "1e18 $ELAPSED_INT"
    exit 0
fi
