    ...
    for version in versions:
        ...
        for i in range(len(solvers)):
            if solvers[i].startswith("nsbrkga"):
                filename = os.path.join(dirname, "num_elites_snapshots/" + instance + "_" + solvers[i] + "_" + version + ".txt")
...
for instance in instances:
    for i in range(len(solvers)):
        if solvers[i].startswith("nsbrkga"):
            for seed in seeds:
                filename = os.path.join(dirname, "num_elites_snapshots/" + instance + "_" + solvers[i] + "_" + str(seed) + ".txt")
                if os.path.exists(filename):
                    with open(filename) as csv_file:
                        data = csv.reader(csv_file, delimiter = " ")
                        j = 0
...
plt.xscale("log")
plt.legend(loc = "best", fontsize = "large")
filename = os.path.join(dirname, "num_elites_snapshots/num_elites_mean_snapshots.png")
plt.savefig(filename, format = "png")
plt.close()
...
plt.xscale("log")
plt.legend(loc = "best", fontsize = "large")
filename = os.path.join(dirname, "num_elites_snapshots/num_elites_quartiles_snapshots.png")
plt.savefig(filename, format = "png")
plt.close()
