...
num_fronts_per_solver = {}
time_per_solver = {}

for solver in solvers:
    num_fronts_per_solver[solver] = []
    time_per_solver[solver] = []
    for i in range(num_snapshots):
        num_fronts_per_solver[solver].append([])
        time_per_solver[solver].append([])

num_fronts_per_snapshot = []

for i in range(num_snapshots):
    num_fronts_per_snapshot.append([])
    for solver in solvers:
        num_fronts_per_snapshot[i].append([])

for instance in instances:
    for i in range(len(solvers)):
        for seed in seeds:
            filename = os.path.join(dirname, "num_fronts_snapshots/" + instance + "_" + solvers[i] + "_" + str(seed) + ".txt")
            if os.path.exists(filename):
                with open(filename) as csv_file:
                    data = csv.reader(csv_file, delimiter = " ")
                    j = 0
                    for row in data:
                        ...
                        num_fronts_per_solver[solvers[i]][j].append(float(row[2]))
                        num_fronts_per_snapshot[j][i].append(float(row[2]))
                        j += 1
                    csv_file.close()
...
for i in range(len(solvers)):
    ...
    for j in range(num_snapshots):
        ...
        y.append(stats.mean(num_fronts_per_solver[solvers[i]][j]))
    plt.plot(x, y, label = solver_labels[solvers[i]], marker = (i + 3, 2, 0), color = colors[i], alpha = 0.80)
plt.xscale("log")
plt.legend(loc = "best", fontsize = "large")
filename = os.path.join(dirname, "num_fronts_snapshots/num_fronts_mean_snapshots.png")
plt.savefig(filename, format = "png")
plt.close()
...
for i in range(len(solvers)):
    ...
    for j in range(num_snapshots):
        x.append(stats.mean(time_per_solver[solvers[i]][j]))
        quantiles = stats.quantiles(num_fronts_per_solver[solvers[i]][j])
...
plt.xscale("log")
plt.legend(loc = "best", fontsize = "large")
filename = os.path.join(dirname, "num_fronts_snapshots/num_fronts_quartiles_snapshots.png")
plt.savefig(filename, format = "png")
plt.close()
